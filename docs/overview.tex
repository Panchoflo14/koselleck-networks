...
Six limitations bound what can be claimed today. All six are stated
...
own thinness after 1780 also still means the 1780-1810 window leans
American-heavy even with the supplement in place. Per-region results
...
is OCR-derived, and OCR text carries its own artifacts. One class - an
original line-break hyphen left sitting in the text (\emph{``utrum-
que''} for \emph{utrumque}) - was found and fixed. A second, distinct
class was not: the OCR sometimes drops the line-break hyphen entirely
and leaves a bare space instead (\emph{``par ticulars''} for
\emph{particulars}), with no punctuation signal left to detect it by.
This produces whole communities that are nothing but word fragments
rather than a real topic - confirmed directly in the 1810-1830 period,
at least fifteen such communities, some several hundred words large. The
labeling pass handles this honestly (routed to ``Structural /
Uncertain,'' not mislabeled), but the underlying vocabulary and network
metrics for 1810-1830, 1830-1850, and 1870-1890 are still diluted by it.
A real fix, not yet built, most plausibly needs a dictionary check
rather than a punctuation pattern.
\end{claimbox}

\begin{claimbox}
\claimlabel{A separate, unconfirmed TCP artifact.} A third word-splitting
pattern, distinct from the OCR issue above, turned up in a handful of
TCP-covered periods (1510-1650) while raising the labeling resolution
(next box) - TCP is manually keyed, not scanned, so it cannot be the
same bug. Words are missing a ``con-''/``com-'' prefix (\emph{mytted}
for \emph{committed}, \emph{uersation} for \emph{conversation}), most
likely from how TCP's own markup for a scribal abbreviation mark gets
handled during extraction - not confirmed, not fixed, flagged rather
than guessed at.
\end{claimbox}

\begin{claimbox}
\claimlabel{Labeling resolution, raised once already.} The webapp
displays and labels communities at one fixed resolution, separate from
the sweep every quantitative claim is tested across. It was 1.0 until
the British Library supplement pushed some periods' vocabularies past
170{,}000 words and the largest community past 24{,}000 words - too
large and mixed for any label to describe. Raised to 4.0 after a
concrete check: the community holding ``system'' in the worst-affected
period stayed an unstructured mix at 1.0 through 3.0, and only split into
a real, nameable theme (civil administration and property law) at 4.0.
The resolution sweep used for the actual claim was extended to match (up
to 4.0). Expect this to need raising again as the corpus grows further -
it scales with vocabulary size, it is not a fixed constant.
\end{claimbox}

\begin{claimbox}
\claimlabel{Label reliability.} Every community carries a short
plain-English name, generated by reading the community's most
representative words. A label is inherited, not re-read, for as long as
the same alignment that decides ``moved''/``stayed'' maps a community
back to a predecessor - but only for up to three consecutive periods
(\code{MAX\_INHERITANCE\_CHAIN}), a cap added 2026-08-07 after a
collaborator-reported case: a community genuinely Dutch/German text at
1510-1530 was still labeled that way fifteen periods (~300 years) later,
by which point its real content had passed through seven unrelated
themes and landed on church governance - the structural ``stayed'' claim
was correct every step, only the unread label was wrong. Past the cap, a
fresh read is forced, keeping the same tracked lineage (\code{stayed}
still means what it always meant) while updating the description - shown
in the tool as a ``stayed'' transition whose label was just
re-examined, distinct from one that genuinely never changed. Every fresh
read, capped or genesis, is still a single, unvalidated pass - useful
for orientation, not for citation. Separately, roughly half of
communities carry the ``Structural / Uncertain'' lane rather than a
topical one (down from 70\% before the labeling resolution was raised,
see below), because the corpus is multilingual (English, Latin, Law
French, Welsh, Scots, and more) and early modern, and a detected
